...
$('recordForm').onsubmit=async e=>{e.preventDefault();const level=S.levels.find(l=>l.id===$('recordLevel').value);const player=S.players.find(p=>p.id===$('recordPlayer').value);if(!player||!level)return;const {data:earned,error}=await sb.rpc('add_victory',{p_player_id:player.id,p_level_id:level.id,p_progress:100,p_video_url:$('recordVideo').value.trim()||null});if(error)alert(error.message);else{const patch={};if($('recordAttempts').value.trim())patch.attempts=Number($('recordAttempts').value);if($('recordSeconds').value.trim())patch.completion_seconds=Number($('recordSeconds').value);if(Object.keys(patch).length){const existing=await sb.from('records').select('id').eq('player_id',player.id).eq('level_id',level.id).single();if(existing.data?.id)await sb.from('records').update(patch).eq('id',existing.data.id)}$('recordVideo').value='';$('recordAttempts').value='';$('recordSeconds').value='';await load();msg(`🏆 ${esc(player.name)} now has the victory for #${level.rank} ${esc(level.name)} — <strong>+${Number(earned)||pointsFor(level)} points</strong> immediately.`,true)}};
$('newLevelBtn').onclick=newLevel;if($('bulkApply'))$('bulkApply').onclick=bulkApply;if($('bulkDelete'))$('bulkDelete').onclick=bulkDelete;$('levelFilter').oninput=renderLevels;$('adminSection').onchange=renderLevels;$('recordLevel').onchange=updateRecordPoints;
...
function updateBulkCount(){const n=document.querySelectorAll('.bulkLevel:checked').length;if($('bulkCount'))$('bulkCount').textContent=`${n} selected`}
async function bulkApply(){const ids=[...document.querySelectorAll('.bulkLevel:checked')].map(x=>x.value);if(!ids.length)return alert('Select at least one level.');const difficulty=$('bulkDifficulty').value.trim()||null,status=$('bulkStatus').value.trim()||null;for(const id of ids){const {error}=await sb.from('levels').update({difficulty,status}).eq('id',id);if(error)return alert(error.message)}alert(`Updated ${ids.length} levels.`);await load()}
async function bulkDelete(){const ids=[...document.querySelectorAll('.bulkLevel:checked')].map(x=>x.value);if(!ids.length)return alert('Select at least one level.');if(!confirm(`Delete ${ids.length} selected levels? Victories/history will cascade.`))return;for(const id of ids){const {error}=await sb.from('levels').delete().eq('id',id);if(error)return alert(error.message)}alert(`Deleted ${ids.length} levels.`);await load()}
if($('newsForm'))$('newsForm').onsubmit=async e=>{e.preventDefault();const title=$('newsTitle').value.trim(),body=$('newsBody').value.trim();if(!title||!body)return;const {error}=await sb.from('changelog').insert({title,body});if(error)return alert(error.message);$('newsTitle').value='';$('newsBody').value='';alert('Published to the live feed.');};
...
// V13 Hall curator
async function loadHallAdmin(){if(!$('hallForm'))return;const [p,l,h]=await Promise.all([sb.from('players').select('id,name').order('name'),sb.from('levels').select('id,rank,name').order('rank'),sb.from('hall_entries').select('*').order('created_at',{ascending:false})]);if(p.error||l.error||h.error)return;$('hallPlayer').innerHTML='<option value="">None</option>'+(p.data||[]).map(x=>`<option value="${x.id}">${esc(x.name)}</option>`).join('');$('hallLevel').innerHTML='<option value="">None</option>'+(l.data||[]).map(x=>`<option value="${x.id}">#${x.rank} · ${esc(x.name)}</option>`).join('');$('hallAdminList').innerHTML=(h.data||[]).map(x=>`<div class="adminRow row between"><span><b>${esc(x.title)}</b><span class="meta"> · ${esc(x.category)}</span></span><button class="danger" onclick="deleteHall('${x.id}')">Delete</button></div>`).join('')||'<p class="meta">No custom exhibits yet.</p>'}
async function deleteHall(id){if(!confirm('Delete this Hall exhibit?'))return;const {error}=await sb.from('hall_entries').delete().eq('id',id);if(error)alert(error.message);else loadHallAdmin()}
if($('hallForm'))$('hallForm').onsubmit=async e=>{e.preventDefault();const payload={category:$('hallCategory').value.trim(),title:$('hallTitle').value.trim(),body:$('hallBody').value.trim(),year:Number($('hallYear').value)||null,player_id:$('hallPlayer').value||null,level_id:$('hallLevel').value||null};const {error}=await sb.from('hall_entries').insert(payload);if(error)return alert(error.message);$('hallForm').reset();await loadHallAdmin();alert('🏛️ Exhibit added to the Hall of Fame.')};
const oldLoadV13=load;load=async function(){await oldLoadV13();if($('hallForm'))await loadHallAdmin()};
