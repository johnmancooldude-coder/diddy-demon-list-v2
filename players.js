...
async function boot(){
 ...
 (data.map((x,i)=>{
  ...
  const last=wins.map(r=>map[r.level_id]).filter(Boolean).sort((a,b)=>a.rank-b.rank)[0], form=playerForm(wins);const formScore=Math.max(0,Math.min(100,50+(form.recentWins-form.previousWins)*10+form.recentWins*4));const title=formScore>=85?'🔥 MOMENTUM MONSTER':wins.length>=50?'💀 DEMON GRINDER':top10>=10?'👑 LIST ROYALTY':wins.length>=10?'⚡ ACTIVE THREAT':'👤 DIDDY PLAYER';
