function v21FormScore(w){const now=Date.now(),d=864e5;let r=0,p=0;w.forEach(x=>{const a=(now-new Date(x.created_at).getTime())/d;if(a>=0&&a<=1)r++;else if(a>1&&a<=2)p++});return Math.max(0,Math.min(100,50+(r-p)*10+r*4));}
...
async function boot(){
 ...
 (data.map((x,i)=>{
  ...
  const last=wins.map(r=>map[r.level_id]).filter(Boolean).sort((a,b)=>a.rank-b.rank)[0], form=playerForm(wins);const top10=wins.filter(r=>(map[r.level_id]?.rank||999)<=10).length;const formScore=Math.max(0,Math.min(100,50+(form.recentWins-form.previousWins)*10+form.recentWins*4));const title=formScore>=85?'🔥 MOMENTUM MONSTER':wins.length>=50?'💀 DEMON GRINDER':top10>=10?'👑 LIST ROYALTY':wins.length>=10?'⚡ ACTIVE THREAT':'👤 DIDDY PLAYER';
  return `<a class="playerRankCard" href="player.html?id=${x.id}"><div class="playerRank">#${i+1}</div><div class="playerAvatar">${esc((x.name||'?').slice(0,2).toUpperCase())}</div><div class="playerMain"><strong>${esc(x.name)}</strong><div class="meta">${wins.length} victories · best ${best} · avg ${x.average_placement??'—'}</div><div class="rankMeter"><span style="width:${pct}%"></span></div>${last?`<small class="meta">Top victory: #${last.rank} ${esc(last.name)}</small>`:''}<div class="playerBadges"><span class="tag">⚡ ${power} POWER</span><span class="tag xpTag">LV ${lvl} · ${xp} XP</span><span class="tag momentumTag ${form.cls}" title="${esc(form.title)}">${form.label}</span><span class="tag">FORM ${formScore}</span><span class="tag">${title}</span></div><small class="formDetail">${esc(form.detail)}</small></div><div class="playerPoints"><strong>${Number(x.total_points||0).toLocaleString()}</strong><small>points</small></div></a>`
